...
DIANN_pic <- function(dpi = 600,
  ...
  for (t in 1:length(sheets)) {
    ...
    yion <- theoretical %>% select(c("amino","y","charge","ymw")) %>% setnames(c("y","ymw"),c("ion","mw"))
    yion$charge <- max(yion$charge)-yion$charge+1
    
    theoretical <- rbind(theoretical %>% select(c("amino","b","charge","bmw")) %>% setnames(c("b","bmw"),c("ion","mw")),
                         theoretical %>% select(c("amino","y","charge","ymw")) %>% setnames(c("y","ymw"),c("ion","mw"))) %>% 
      .[,`:=`(`m/z`=(mw+(charge-1)*1.00794)/charge,
              type=str_sub(ion,1,1),
              frag=paste(ion,str_dup("+",times = charge),sep = ""))]
    fragment <- theoretical %>% select(c("frag","m/z")) %>% arrange(by = `m/z`)
    ...
    # annotation --------------------------------------------------------------------
    annotation <- theoretical %>% select(c("amino")) %>% .[1:(nrow(theoretical)/2)] %>% 
      .[,`:=`(bion=paste("b",seq(1:nrow(.)),sep=""),
              yion=paste("y",seq(1:nrow(.)) %>% rev(),sep=""),
              x = seq(1,nrow(.)),y = 0)]
    annotation <- annotation[bion %in% data$ylabel,xlabel:=x][yion %in% data$ylabel,ylabel:=x]
    ...
    p1 <- ggplot(data,aes(x = `m/z`,y = Intensity))+
      ...
      scale_y_continuous(expand = c(0,0),limits = c(0,data[group=="b"|group=="y"] %>% select(Intensity) %>% max(na.rm = T)*1.5))+
